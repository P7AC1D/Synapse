...
class TradingEnv(gym.Env, EzPickle):
    ...
    def __init__(self, data: pd.DataFrame, initial_balance: float = 500.0,
        ...
        # Observation space = features + position_type + unrealized_pnl + hold_time
        feature_count = len(self.raw_data.columns) + 3  # +3 for position_type, unrealized_pnl, hold_time
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, bool, Dict[str, Any]]:
        ...
        # Open positions will be handled by the position details in metrics

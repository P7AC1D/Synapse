...
class BalanceEvalCallback(BaseCallback):
    ...
    def __init__(self, eval_env, eval_freq=10000, best_model_save_path=None, 
        ...
        # Evaluation logs
        self.eval_results = []
        # Extra callback after evaluation
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            # Save best model based on BALANCE (not reward)
            ...
            # Run callback after evaluation (like StopTrainingOnNoModelImprovement)
            if self.callback_after_eval is not None:
                # Ensure the callback has the required attributes
                if hasattr(self.callback_after_eval, "on_step"):
                    self.callback_after_eval.model = self.model
                    self.callback_after_eval.n_calls = self.n_calls
                    self.callback_after_eval.num_timesteps = self.num_timesteps
                    # Important: pass the mean BALANCE to the callback, not reward
                    self.callback_after_eval.last_mean_reward = mean_balance
                    continue_training = self.callback_after_eval.on_step()
...
class DQNTrainer:
    ...
    def final_training(self, best_params, timesteps=5000000):
        """Conduct final training with the best parameters."""
        print("Starting final training with best parameters...")
        
        # Extract environment parameters (use fixed values since they weren't optimized)
        env_params = {
            'bar_count': 50,  # Use fixed value since this wasn't optimized
            'normalization_window': 100  # Use fixed value since this wasn't optimized
        }
        
        # Extract model parameters from the optimization results
        model_params = {
            ...
            'train_freq': 8,  # Adding train_freq as in original code
        }
        ...
        # Save final model
        final_model_path = f"{self.models_dir}/dqn_forex_final_{self.seed}"
        model.save(final_model_path)
        print(f"Final model saved to {final_model_path}")
        
        # Save best parameters (include both env and model params)
...
if __name__ == "__main__":
    ...
    # Run full pipeline
    # You can adjust the number of trials and final training timesteps
    model, model_path = trainer.run_full_pipeline(
        broad_trials=20,         # Number of trials for broad search
        narrow_trials=15,        # Number of trials for narrow search
        final_timesteps=3000000  # Timesteps for final training
    )
